globals [ max-sheep wolves-count sheep-count real-wolves-count real-sheep-count]  ; don't let the sheep population grow too large
...
to go
  ...
  if not any? wolves and count sheep > max-sheep [
    ...
    while [length sheep-count < 1000]
    [
      set sheep-count lput max-sheep  sheep-count
    ]
        while [length sheep-count < 1000]
    [
       set wolves-count lput 0 wolves-count
    ]

  user-message "The sheep have inherited the earth" stop
  ]
  ask sheep [
    move
  ...
  ;;keep counting!
  set wolves-count lput ( count wolves ) wolves-count
  set sheep-count lput ( count sheep ) sheep-count
...
to-report error-wolves

  if length wolves-count < 1000
  [ report "na" ]

  ;; life is too short to learn reduce/double foreach, so let's use range
  let sum-of-errors 0
  foreach range(1000) [
    t ->  set sum-of-errors (sum-of-errors + (item t wolves-count - item t real-wolves-count) ^ 2 )
  ]
  report sqrt sum-of-errors

end


to-report error-sheep

  if length sheep-count < 1000
  [ report "na" ]

  let sum-of-errors 0
  foreach range(1000) [
    t ->  set sum-of-errors (sum-of-errors + (item t sheep-count - item t real-sheep-count) ^ 2 )
  ]
  report sqrt sum-of-errors
...
to calibration-setup
  ...
  ;; read target files...
  file-open "./targets/sheep.txt"
  set real-sheep-count (file-read)
  file-close


  file-open "./targets/wolves.txt"
  set real-wolves-count (file-read)
  file-close

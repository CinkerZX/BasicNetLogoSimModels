;to setup           ; Define a new procedure: Initialization
;  clear-all        ; Command procedure (has a result)
;  reset-ticks      ; Time Step
;end
...
to setup
  ...
  ask n-of num-turtles patches [      ; petch
    sprout 1  [                       ; Generate one turtle on each pach
      set color one-of [ yellow sky ] ; color of the turtle: a list
    ]
  ]
  reset-ticks
end

; reporter(boolean): if the position is good
; Do not change the world, but make judgement
  ; Creat "LOCAL var"
to-report demands-met? [ color-wanted ]
  let all-neighbors turtles-on neighbors
  let similar-neighbors all-neighbors with [
    color = color-wanted
  ]
  let number-wanted count all-neighbors * proportion-wanted
  report count similar-neighbors >= number-wanted
end
